...
#define LEXER_IS_DONE(lexer)                    \
  (lexer->i >= lexer->src.length || lexer->c == '\0')
...
HTMLToken* html_lexer_get_next_token(HTMLLexer* lexer) {
  while (!LEXER_IS_DONE(lexer)) {
    ...
    if (isalnum(lexer->c)) return html_lexer_parse_id(lexer);
    if (isdigit(lexer->c)) return html_lexer_parse_number(lexer);

    switch (lexer->c) {
      case '<': LEXER_TOK(lexer, TOKEN_LT);
      case '>': LEXER_TOK(lexer, TOKEN_GT);
      case '=': LEXER_TOK(lexer, TOKEN_EQUALS);
      case '/': LEXER_TOK(lexer, TOKEN_DIV);
      case '\'':
      case '"': return html_lexer_parse_string(lexer);
      default: { printf("Lexer: Unexpected character `%c`", lexer->c); exit(1); }
    }
  }
...
HTMLToken* html_lexer_parse_id(HTMLLexer* lexer) {
  ...
  while ((isalnum(lexer->c) || lexer->c == '-' || lexer->c == '_') && !(LEXER_IS_DONE(lexer))) {
