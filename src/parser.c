...
HTMLAST *html_parser_parse_compound(HTMLParser *parser, HTMLAST* parent) {
  ...
  while (parser->token->type != HTML_TOKEN_EOF) {
    HTMLAST *child = html_parser_parse_expr(parser, parent);
...
HTMLAST *html_parser_parse_element(HTMLParser *parser, HTMLAST* parent) {
  if (parser->token->type != HTML_TOKEN_LT) return 0;
  ...
  EStr* name = e_fromstr(parser->token->value->value);
  ast->value_str = name;
  ...
  if (!ast->is_end) {
    ...
    if (!ast->is_self_closing) {
      ...
      while (name && child && child_name && !(child->is_end && strcmp(name->value, child_name) == 0)) {
...
HTMLAST *html_parser_parse_raw(HTMLParser *parser, HTMLAST* parent) {
  ...
  if (parser->token)
    html_token_free(parser->token);
  parser->token = tok;
  return html_parser_parse_string_element(parser, parent);
}
...
HTMLAST *html_parser_parse_factor(HTMLParser *parser, HTMLAST* parent) {
  ...
  if (parser->token->type == HTML_TOKEN_LT) {
    return html_parser_parse_element(parser, parent);
  }
  ...
  case HTML_TOKEN_STR:
    left = html_parser_parse_string(parser, parent);
    break;
  case HTML_TOKEN_NUMBER:
    left = html_parser_parse_number(parser, parent);
    break;
  default: {

    if (parser->token->type != HTML_TOKEN_EOF) {
      left = html_parser_parse_raw(parser, parent);
    }
  } break;
  }
